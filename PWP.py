...
def model_timestep(T,S,U,V,z,I,L,E,P,tau_x,tau_y,dt,MLI_flux=None,Ekman_Q_flux=None,use_static_stability=True,use_mixed_layer_stability=True,use_shear_stability=True,use_Ekman_flux=False,use_MLI=False,verbose=False,I1=0.62,I2=None,lambda1=.6,lambda2=20,T0=17,S0=36,rho0=None,alpha=None,beta=None,f=sw.f(40),return_MLD=False):
    ...
    if use_Ekman_flux or use_MLI:
        mld_ind = get_mld_ind(T,S,U,V,z)
        mld = z[mld_ind]
    if use_Ekman_flux:
        Q_Ek_profile = np.zeros(len(z))
        Q_Ek_profile[0:mld_ind] = Ekman_Q_flux/mld*dz; 
        Q_profile += Q_Ek_profile
    if use_MLI:
        Q_MLI_profile = np.zeros(len(z))
        Q_MLI_profile[0:mld_ind] = MLI_flux*mld**2 /mld*dz
        Q_profile += Q_MLI_profile
    ...
    if return_MLD:
        return T,S,U,V,get_mld(T,S,U,V,z)
    else:
        return T,S,U,V
